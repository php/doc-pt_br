#!/usr/bin/env bash
#
# Track changes in php/doc-en and create issues in doc-pt_br
# for files that need translation updates.
#
# How it works:
#   - Fetches all commits on doc-en master from the last 7 days
#   - For each commit, checks if a matching issue already exists in doc-pt_br
...
while read -r SHA; do
  ...
  if [ -n "$UPDATE_LIST" ]; then
    BODY+=$'\n'"**Archivos PT_BR files to update**"$'\n'
    BODY+="$UPDATE_LIST"
  fi
